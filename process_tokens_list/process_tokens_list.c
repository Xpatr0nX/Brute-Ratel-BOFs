...
static void BeaconDataParse(datap * parser, char ** argv, int argc) {
    parser->argv = argv;
    parser->argc = argc;
    parser->current = 1;
}
...
static char * BeaconDataExtract(datap * parser, int * size) {
    if (parser->current < parser->argc) {
        char * val = parser->argv[parser->current];
        if (size) {
            int len = 0;
            while(val[len]) len++;
            *size = len;
...
BOOL PrintTokenInfo(WCHAR **dispatch, HANDLE hToken, DWORD pid, WCHAR* procNameW, HighValueEntry *hvEntry, int hvCount) {
    ...
    if (procNameW == NULL || procNameW[0] == L'\0') {
        ...
        procNameA[0] = '<'; procNameA[1] = 'u'; procNameA[2] = 'n'; procNameA[3] = 'k';
        procNameA[4] = 'n'; procNameA[5] = 'o'; procNameA[6] = 'w'; procNameA[7] = 'n';
        procNameA[8] = '>'; procNameA[9] = '\0';
    } else {
...
void coffee(char **argv, int argc, WCHAR **dispatch) {
    ...
    // Arg checking logic shifted to utilize BRC4's string arrays
    if (argc > 1) {
    ...
    if (enableSeDebug) {
        ...
        BeaconPrintf(CALLBACK_OUTPUT, "[i] skipping seDebugPrivilege (OPSEC)\n");
    }
    ...
    for (i = 0; i < cProcesses; i++) {
        ...
        if (nameLen > 0 && nameLen < MAX_PATH) {
            ...
            if (ADVAPI32$OpenProcessToken(hProcess, TOKEN_QUERY, &hToken)) {
                ...
                BOOL isHV = PrintTokenInfo(dispatch, hToken, pids[i], last_slash, hvSlot, hvCount);
                if (isHV && hvSlot) {
                    hvCount++;
                }
